...
echo "FANTOM_SNAPSHOTS_URI=${_FANTOM_SNAPSHOTS_URI_}" >> /etc/environment
...
echo "FANTOM_NODE_TYPE=${_FANTOM_NODE_TYPE_}" >> /etc/environment
echo "FANTOM_NETWORK=${_FANTOM_NETWORK_}" >> /etc/environment
...
echo "Install FANTOM geth client"

# download fantom client and configuration
mkdir -p /home/bcuser/fantom
cd /home/bcuser/fantom

wget https://github.com/Fantom-foundation/go-opera/archive/refs/tags/v1.1.3-rc.5.tar.gz
tar xvfz v1.1.3-rc.5.tar.gz
cd go-opera-1.1.3-rc.5/

# download network configuration
wget $(curl -s https://api.github.com/repos/bnb-chain/fantom/releases/latest |grep browser_ |grep $FANTOM_NETWORK |cut -d\" -f4)
unzip $FANTOM_NETWORK.zip
...
if [[ "$FANTOM_NODE_TYPE" == "full" ]]; then
  sudo mkdir /home/bcuser/bin
  sudo mv /opt/fantom/rpc-template.sh /home/bcuser/bin/node.sh
fi
