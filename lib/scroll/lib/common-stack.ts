...
export interface ScrollCommonStackProps extends cdk.StackProps {

}

export class ScrollCommonStack extends cdk.Stack {
  AWS_STACKNAME = cdk.Stack.of(this).stackName;
  AWS_ACCOUNT_ID = cdk.Stack.of(this).account;

  constructor(scope: cdkConstructs.Construct, id: string, props: ScrollCommonStackProps) {
    ...
    instanceRole.addToPolicy(new iam.PolicyStatement({
      resources: [`arn:aws:autoscaling:${region}:${this.AWS_ACCOUNT_ID}:autoScalingGroup:*:autoScalingGroupName/scroll-*`],
      actions: ["autoscaling:CompleteLifecycleAction"],
    }));

    new cdk.CfnOutput(this, "Instance Role ARN", {
      value: instanceRole.roleArn,
      exportName: "ScrollNodeInstanceRoleArn",
    });
