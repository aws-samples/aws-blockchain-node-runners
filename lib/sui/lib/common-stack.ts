...
export interface SuiCommonStackProps extends cdk.StackProps {

}

export class SuiCommonStack extends cdk.Stack {
  AWS_STACKNAME = cdk.Stack.of(this).stackName;
  AWS_ACCOUNT_ID = cdk.Stack.of(this).account;

  constructor(scope: cdkConstructs.Construct, id: string, props: SuiCommonStackProps) {
    ...
    instanceRole.addToPolicy(new iam.PolicyStatement({
      resources: [`arn:aws:autoscaling:${region}:${this.AWS_ACCOUNT_ID}:autoScalingGroup:*:autoScalingGroupName/sui-*`],
      actions: ["autoscaling:CompleteLifecycleAction"],
    }));

    new cdk.CfnOutput(this, "Instance Role ARN", {
      value: instanceRole.roleArn,
      exportName: "SuiNodeInstanceRoleArn",
    });
