...
export interface BitcoinCommonStackProps extends cdk.StackProps {
}
...
export class BitcoinCommonStack extends cdk.Stack {
    ...
    constructor(scope: cdkConstructs.Construct, id: string, props: BitcoinCommonStackProps) {
        ...
        instanceRole.addToPolicy(new iam.PolicyStatement({
            resources: [`arn:aws:autoscaling:${region}:${this.AWS_ACCOUNT_ID}:autoScalingGroup:*:autoScalingGroupName/bitcoin-*`],
        ...
        instanceRole.addToPolicy(
            new iam.PolicyStatement({
                resources: [
                    `arn:aws:cloudwatch::${this.AWS_ACCOUNT_ID}:dashboard/bitcoin-*`,
                ],
                actions: ["cloudwatch:PutDashboard", "cloudwatch:GetDashboard"],
            })
        );

        // Allow publishing custom metrics to CloudWatch
        instanceRole.addToPolicy(
            new iam.PolicyStatement({
                resources: ["*"],
